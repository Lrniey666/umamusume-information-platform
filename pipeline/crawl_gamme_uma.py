...
宅宅新聞 — 賽馬娘相關文章爬蟲
目標：https://news.gamme.com.tw/search.php?s=馬娘
輸出：data/raw/gamme_uma_raw.csv（pipe 分隔，UTF-8 BOM）

分頁模式：/page/N/（WordPress 式）

Phase 2c 新增：爬取後同步寫入 NewsData DB（status='raw'）。

用法：
    python pipeline/crawl_gamme_uma.py
    python pipeline/crawl_gamme_uma.py --max-pages 5
"""
import re
import time
import random
...
SOURCE       = 'gamme'
# 搜尋頁使用 Google Custom Search（需 JS 渲染），改用 tag 頁（靜態 HTML）
TAG_BASE_URL = 'https://news.gamme.com.tw/tag/%E8%B3%BD%E9%A6%AC%E5%A8%98'
BASE_URL     = 'https://news.gamme.com.tw'
MAX_PAGES    = 30

# 廣告/推薦欄位的 parent class，排除用
_AD_CLASSES  = {'anad', 'adwrpt', 'xys1', 'sidead_300250', 'left_sky', 'nav'}
MAX_ARTICLES = 0
MAX_RETRIES  = 3
DELAY_MIN    = _env_float('CRAWLER_DELAY_MIN', 1.0)
DELAY_MAX    = _env_float('CRAWLER_DELAY_MAX', 1.8)

ROOT_DIR  = Path(__file__).parent.parent
OUT_CSV   = ROOT_DIR / 'data' / 'raw' / 'gamme_uma_raw.csv'
FAIL_CSV  = ROOT_DIR / 'data' / 'raw' / 'gamme_uma_failed.csv'
...
def classify_title(title: str) -> str:
    if any(k in title for k in ['活動', '周年', '慶典', '獎勵']):
        return '活動'
    if any(k in title for k in ['卡池', '限定', '抽卡', 'Gacha']):
        return '卡池'
    if any(k in title for k in ['賽事', '競技', '錦標', '排名']):
        return '競賽'
    if any(k in title for k in ['更新', '維護', '系統', 'BUG']):
        return '系統'
    return '其他'
...
def collect_links(max_pages: int, crawled_ids: set) -> list[dict]:
    ...
    for page in range(1, max_pages + 1):
        # tag 頁分頁格式：/tag/賽馬娘/page/N/
        url = TAG_BASE_URL if page == 1 else f'{TAG_BASE_URL}/page/{page}/'
        print(f'  標籤頁第 {page} 頁：{url}')
        soup = fetch(url)
        if not soup:
            break

        # 標籤文章列表：div[class^="List"] h3 a（排除頁首最新文章側欄）
        art_links = soup.select('div[class^="List"] h3 a[href]')
        ...
        for a in art_links:
            ...
            item_id = f'gamme_{m.group(1)}' if m else f'gamme_{abs(hash(full_url)) % 10**8}'
...
def parse_article(soup: BeautifulSoup, entry: dict) -> dict:
    ...
    # 日期：<span class="postDate" content="YYYY-MM-DD">
    date = ''
    pd_tag = soup.select_one('.postDate[content]')
    if pd_tag:
        raw = pd_tag.get('content', '').strip()
        try:
            date = pd.to_datetime(raw).strftime('%Y-%m-%d')
        except Exception:
            date = raw[:20]
    else:
        for sel in ['time[datetime]', '.post-date', '.entry-date']:
            tag = soup.select_one(sel)
            if tag:
    ...
    # 內文：<div class="entry">
    content = ''
    for sel in ['.entry', '.entry-content', '.post-content']:
        tag = soup.select_one(sel)
        if tag:
    ...
    photo_link = ''
    # 嘗試縮圖
    thumb = soup.select_one('.post-thumbnail img, .wp-post-image')
    if thumb:
        photo_link = thumb.get('src') or thumb.get('data-src') or ''
    if not photo_link:
...
def main():
    ...
    try:
        r = requests.get(TAG_BASE_URL, headers=HEADERS, timeout=15)
        r.raise_for_status()
