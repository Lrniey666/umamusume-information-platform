...
ETtoday 新聞雲 — 賽馬娘相關文章爬蟲
目標：https://game.ettoday.net/search.php?keyword=馬娘
輸出：data/raw/ettoday_uma_raw.csv（pipe 分隔，UTF-8 BOM）

注意：若搜尋結果頁需 JS 渲染，可在 Phase 1 探查後改用 Playwright。
     本腳本預設使用 requests + BeautifulSoup（靜態爬取）。

Phase 2c 新增：爬取後同步寫入 NewsData DB（status='raw'）。

用法：
    python pipeline/crawl_ettoday_uma.py
    python pipeline/crawl_ettoday_uma.py --max-pages 5
"""
import re
import time
import random
...
SOURCE       = 'ettoday'
# 第 1 頁用 keyword 參數，第 2 頁起改用 type=search&keyword=...&idx=1&page=N
SEARCH_URL   = 'https://game.ettoday.net/search.php?type=search&keyword=%E9%A6%AC%E5%A8%98&idx=1&page=1'
BASE_URL     = 'https://game.ettoday.net'
MAX_PAGES    = 50
MAX_ARTICLES = 0
MAX_RETRIES  = 3
DELAY_MIN    = _env_float('CRAWLER_DELAY_MIN', 1.2)
DELAY_MAX    = _env_float('CRAWLER_DELAY_MAX', 2.2)

ROOT_DIR  = Path(__file__).parent.parent
OUT_CSV   = ROOT_DIR / 'data' / 'raw' / 'ettoday_uma_raw.csv'
FAIL_CSV  = ROOT_DIR / 'data' / 'raw' / 'ettoday_uma_failed.csv'
...
HEADERS = {
    ...
    'Referer':         'https://game.ettoday.net/',
...
def classify_title(title: str) -> str:
    if any(k in title for k in ['活動', '周年', '慶典', '獎勵']):
        return '活動'
    if any(k in title for k in ['卡池', '限定', '抽卡', 'SR', 'SSR']):
        return '卡池'
    if any(k in title for k in ['賽事', '競技', '錦標', '排名', '決賽']):
        return '競賽'
    if any(k in title for k in ['更新', '維護', '系統', 'BUG', '修復']):
        return '系統'
    return '其他'
...
def collect_links(max_pages: int, crawled_ids: set) -> list[dict]:
    ...
    BASE_PAGED = 'https://game.ettoday.net/search.php?type=search&keyword=%E9%A6%AC%E5%A8%98&idx=1&page='

    for page in range(1, max_pages + 1):
        url = BASE_PAGED + str(page)
        print(f'  搜尋第 {page} 頁：{url}')
        soup = fetch(url)
        if not soup:
            break

        # ETtoday 2026 實際結構：文章卡片為 div.box.clearfix > a[href]
        # 每篇文章的 a 標籤會出現兩次（縮圖 + 標題），以 href 去重
        raw_links = soup.select('.box.clearfix a[href]')
        ...
        for a in raw_links:
            ...
            if not href or href in seen:
                continue
            if 'ettoday.net' not in href:
                continue
            seen.add(href)
            full_url = href if href.startswith('http') else urljoin(BASE_URL, href)
            m = re.search(r'/article/(\d+)', full_url)
            item_id = f'ettoday_{m.group(1)}' if m else f'ettoday_{abs(hash(full_url)) % 10**8}'
            if item_id in crawled_ids:
                continue
            # 取文字較長的那次（標題 a，非縮圖 a）
            title = a.get_text(strip=True)
            if not title:
                # 縮圖連結無文字，找同一 .box 內的 h3
                box = a.find_parent(class_='box')
                h3 = box.find('h3') if box else None
...
def parse_article(soup: BeautifulSoup, entry: dict) -> dict:
    title = entry['title']
    for sel in ['h1.title', '.title h1', 'h1']:
    ...
    date = ''
    for sel in ['.date', 'time[datetime]', '.story-date']:
        tag = soup.select_one(sel)
        if tag:
    ...
    content = ''
    for sel in ['.story', '.article-body', '.main-content', '.context']:
        tag = soup.select_one(sel)
        if tag:
